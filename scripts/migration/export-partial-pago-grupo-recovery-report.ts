...
const OUTPUT_PATH = path.join(process.cwd(), 'exports', 'partial-recovery-2026-04-27-vicky-el-nayar.csv');
...
type CsvRow = {
  ...
  DE: string;
  adelantoSaliente: string;
  puedeAutomatico: 'SI' | 'NO';
  requiereDecisionManual: 'SI' | 'NO';
  riesgoDuplicado: 'SI' | 'NO';
  motivo: string;
  decisionManual: '';
  montoPago: '';
  montoFalla: '';
...
async function loadReadOnlyData(client: PrismaClient) {
  ...
  const [
    ...
    query<GrupoRow>(
      client,
      Prisma.sql`select "id", "name" from "Grupo" where "id" = ${TARGET.promotoriaId}`,
    ),
    query<AuditLogRow>(
      client,
      Prisma.sql`
  ...
  const clienteIds = [
    ...new Set(
      creditos
        .flatMap((credito) => [credito.clienteId, credito.avalClienteId])
  ...
  const [
    ...
    queryByIds<ClienteRow>(
      ...
      'id',
      clienteIds,
      Prisma.sql`select "id", "code", "fullName"`,
    ),
...
function buildExpectedRows(input: {
  ...
  for (const credito of data.creditos) {
    const schedules = (schedulesByCreditoId.get(credito.id) ?? [])
      .slice()
    ...
    const deAmount =
      isWithinActiveWindow && operationalScope === 'active' && targetSchedule
...
function classifyMissingRow(input: {
  ...
  if (
    ...
    duplicateReasons.push('Ya existe PaymentAllocation CURRENT para el calendario de la fila.');
  }
  if (input.row.scheduleId && (input.defaultsByScheduleId.get(input.row.scheduleId) ?? []).length > 0) {
    duplicateReasons.push('Ya existe DefaultEvent para el calendario de la fila.');
  }
  if (
    ...
    duplicateReasons.push('Ya existe RecoveryEvent para la falla ancla en la fecha.');
  }
  if ((input.advancesOnDateByCreditoId.get(input.row.creditoId) ?? []).length > 0) {
    duplicateReasons.push('Ya existe AdvanceEvent para el credito en la fecha.');
  }
  if (
    ...
    duplicateReasons.push('Ya existe PaymentAllocation EXTRA_WEEK para la fila.');
  }
  if (allocationsOnDate.length > 0) {
    duplicateReasons.push('Ya existe al menos una PaymentAllocation en la fecha para este credito.');
  }
  ...
  const reason = duplicateReasons.length
    ? duplicateReasons.join(' ')
    : canImpactAutomatically
      ? 'Inferible solo para snapshot/cierre; no crear eventos financieros automaticamente sin revision.'
      : 'Requiere decision manual antes de cualquier escritura.';
...
function toCsvRows(missingRows: MissingRow[]): CsvRow[] {
  return missingRows.map((row) => ({
    ...
    tiposPosibles: row.types.join(' | '),
    DE: formatMoney(row.deAmount),
    adelantoSaliente: formatMoney(row.outgoingAdvanceAmount),
    puedeAutomatico: row.canImpactAutomatically ? 'SI' : 'NO',
    requiereDecisionManual: row.requiresManualDecision ? 'SI' : 'NO',
    riesgoDuplicado: row.duplicateReasons.length ? 'SI' : 'NO',
    motivo: row.reason,
    decisionManual: '',
    montoPago: '',
    montoFalla: '',
...
function summarizeRows(rows: ExpectedRow[]) {
  return {
    rows: rows.length,
    deAmount: sumMoney(rows, (row) => row.deAmount),
    failureCandidateAmount: sumMoney(rows.filter((row) => row.collectibleAmount > 0.001), (row) => row.collectibleAmount),
    recoveryAmount: sumMoney(rows, (row) => row.recoveryAmountAvailable),
    advanceIncomingAmount: sumMoney(rows, (row) => row.advanceAmountAvailable),
    advanceOutgoingAmount: sumMoney(rows, (row) => row.outgoingAdvanceAmount),
    extraWeekAmount: sumMoney(rows, (row) => row.extraWeekAmount),
    baseTotalToDeliver: sumMoney(rows, (row) => row.deAmount - row.outgoingAdvanceAmount),
  };
}
...
function printSummary(input: {
  ...
  const impactPayload = getJsonRecord(input.data.impactAudits[0]?.afterJson);
  const auditRowsCount = Array.isArray(impactPayload?.rowsSnapshot) ? impactPayload.rowsSnapshot.length : 0;
  const duplicateRiskRows = input.missingRows.filter((row) => row.duplicateReasons.length);

  console.log('\nReporte de recuperacion manual generado');
  console.table([
    {
      promotoriaId: TARGET.promotoriaId,
      promotoria: input.data.grupos[0]?.name ?? null,
  ...
  console.table([
    {
      expectedRows: input.expectedRows.length,
      impactedRows: input.impactedRows.length,
      missingRows: input.missingRows.length,
      expectedRowsDiagnostico: EXPECTED_COUNTS.expectedRows,
      impactedRowsDiagnostico: EXPECTED_COUNTS.impactedRows,
      missingRowsDiagnostico: EXPECTED_COUNTS.missingRows,
      pagoGrupoImpactCount: input.data.impactAudits.length,
      pagoGrupoImpactRowsSnapshot: auditRowsCount,
      pagoGrupoLiquidacionCount: input.data.liquidationAudits.length,
    },
  ]);

  console.log('\nImportes faltantes');
  console.table([{ label: 'Faltante para captura manual', ...summarizeRows(input.missingRows) }]);
  ...
  if (
    input.expectedRows.length !== EXPECTED_COUNTS.expectedRows ||
    input.impactedRows.length !== EXPECTED_COUNTS.impactedRows ||
    input.missingRows.length !== EXPECTED_COUNTS.missingRows
  ) {
    console.warn(
      '\nADVERTENCIA: Los conteos actuales no coinciden con el diagnostico previo. Revisar el CSV antes de cualquier recuperacion.',
    );
  }
...
async function main() {
  ...
  try {
    ...
    const partialStartedAt =
      [...partialPaymentEvents, ...partialDefaultEvents]
    ...
    const paymentById = new Map(data.paymentEvents.map((payment) => [payment.id, payment]));
    ...
    const csv = buildCsv(toCsvRows(missingRows));
