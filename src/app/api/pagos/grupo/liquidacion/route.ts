...
import { saveGrupoLiquidacionSchema } from '@/server/validators/pago';
import { savePagoGrupoLiquidacion } from '@/server/services/pagos-service';
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = saveGrupoLiquidacionSchema.parse(payload);
    const result = await savePagoGrupoLiquidacion(parsed, session.user.id);
