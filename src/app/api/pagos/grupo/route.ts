...
import { impactPagoGrupoSchema } from '@/server/validators/pago';
import { impactPagoGrupo } from '@/server/services/pagos-service';
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = impactPagoGrupoSchema.parse(payload);
    const result = await impactPagoGrupo(parsed, session.user.id);
