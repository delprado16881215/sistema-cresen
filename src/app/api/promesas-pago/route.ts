...
import { createPromesaPago, listPromesasPago } from '@/server/services/promesas-pago-service';
import {
  createPromesaPagoSchema,
  listPromesasPagoSchema,
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createPromesaPagoSchema.parse(payload);
    const created = await createPromesaPago(parsed, session.user.id);
