...
import { createVisitaCampo, listVisitasCampo } from '@/server/services/visitas-campo-service';
import {
  createVisitaCampoSchema,
  listVisitasCampoSchema,
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createVisitaCampoSchema.parse(payload);
    const created = await createVisitaCampo(parsed, session.user.id);
