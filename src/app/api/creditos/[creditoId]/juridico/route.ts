...
import { sendCreditoToLegal } from '@/server/services/credito-legal-service';
import { sendCreditoToLegalSchema } from '@/server/validators/credito';
...
export async function POST(
  ...
  try {
    ...
    const parsed = sendCreditoToLegalSchema.parse(payload);

    const result = await sendCreditoToLegal({
      creditoId,
      payload: parsed,
