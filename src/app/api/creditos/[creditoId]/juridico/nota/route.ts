...
import { addCreditoLegalNote } from '@/server/services/credito-legal-service';
import { createCreditoLegalNoteSchema } from '@/server/validators/juridico';
...
export async function POST(
  ...
  try {
    ...
    const parsed = createCreditoLegalNoteSchema.parse(payload);

    const result = await addCreditoLegalNote({
      creditoId,
      payload: parsed,
